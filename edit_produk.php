...
// Fetch product data
try {
    $query = "SELECT * FROM produk WHERE id = ? LIMIT 1";
    ...
    $_SESSION['error'] = 'Terjadi kesalahan saat mengambil data produk!';
    header('Location: index.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama_produk)) {
        ...
        // Handle image upload
        $gambar_produk = $produk['gambar_produk']; // Keep old image by default
                ...
                // Upload file
                    ...
                    if (move_uploaded_file($file['tmp_name'], $upload_path)) {
                        ...
                        if (!empty($produk['gambar_produk'])) {
                            $old_path = $gambar_dir . '/' . $produk['gambar_produk'];
        ...
        if (empty($error)) {
            try {
                $query = "UPDATE produk SET nama_produk = ?, deskripsi = ?, harga_beli = ?, harga_jual = ?, gambar_produk = ? 
                          WHERE id = ?";
                $stmt = $koneksi->prepare($query);
...
<head>
    ...
    <meta name="description" content="Edit Produk - Sistem Manajemen Data Produk">
                        ...
                        <form method="POST" action="" enctype="multipart/form-data">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="nama_produk" name="nama_produk" 
                                       ...
                                       value="<?= htmlspecialchars($produk['nama_produk'], ENT_QUOTES, 'UTF-8'); ?>">
                            </div>

                            <div class="mb-3">
                                <label for="deskripsi" class="form-label">Deskripsi Produk</label>
                                <textarea class="form-control" id="deskripsi" name="deskripsi" rows="4" 
                                          placeholder="Masukkan deskripsi produk"><?= htmlspecialchars($produk['deskripsi'], ENT_QUOTES, 'UTF-8'); ?></textarea>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <?php if (!empty($produk['gambar_produk']) && file_exists('gambar/' . $produk['gambar_produk'])): ?>
                                    <div class="mb-2">
                                        <label class="text-muted small">Gambar saat ini:</label><br>
                                        <img src="gambar/<?= htmlspecialchars($produk['gambar_produk'], ENT_QUOTES, 'UTF-8'); ?>" 
                                             alt="Current image" class="current-image img-thumbnail">
                                    </div>
                                <?php endif; ?>
                                
                                <input type="file" class="form-control" id="gambar_produk" name="gambar_produk" 
                                       accept="image/*" onchange="previewImage(event)">
                                <small class="text-muted">Format: JPG, JPEG, PNG, GIF (Max 2MB) - Kosongkan jika tidak ingin mengubah gambar</small>
                                <img id="imagePreview" class="preview-image img-thumbnail" alt="Preview" style="display:none;">
                            </div>
